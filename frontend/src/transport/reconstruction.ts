...
export type ModelAsset={id:string;displayName?:string;poiId:string|null;campusId:string;imageId:string;jobId:string;sha256:string;extents:number[];bounds:number[][];vertices:number;faces:number;quality:string;qualityReview?:QualityReview;publicFileUrl?:string;generatedAt?:string;reconstructionScope?:string;sourceImages?:{url:string;title?:string;sourceUrl?:string}[];cameras?:ReconstructedCamera[];imageIds?:string[];viewCount?:number;inputMode?:'single_image'|'multi_view'|'concept_text';feedback?:ReconstructionFeedback;preprocessing?:{profile:string;warnings:string[]}};
