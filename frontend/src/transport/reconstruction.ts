export type ModelAsset={id:string;poiId:string;campusId:string;imageId:string;jobId:string;sha256:string;extents:number[];bounds:number[][];vertices:number;faces:number;quality:string;preprocessing?:{profile:string;warnings:string[]}};
...
export type ReconstructionState={ready:boolean;textReady?:boolean;images:any[];jobs:any[];assets:ModelAsset[];layouts:ModelLayout[];runs:any[]};
