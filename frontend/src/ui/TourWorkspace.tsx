import {waitObserved} from '../transport/harness';
...
interface Props {
 onExport?(tour:TourSession,format:'markdown'|'json'):void;
 registerHarnessRoute?(handler:((poiId:string,signal:AbortSignal,deadline:string)=>Promise<Record<string,unknown>>)|null):void;
...
export function TourWorkspace({onExport,registerHarnessRoute,campus,voice,caption,narration,onStop,onExplain,onCampus,onReadRoute,memory,onMemory,registerCancel,registerText,registerMapFocus,selectedPoi,onExplainPoi,onRouteChange,onSelectPoi,panelOpen,onPanelClose,speechStatus,presentation,explainLabel}:Props){
 ...
 const appliedRouteRef=useRef(appliedRoute);appliedRouteRef.current=appliedRoute;
 useEffect(()=>{registerHarnessRoute?.(async(poiId,signal,deadline)=>{const poi=pois.find(p=>p.id===poiId);if(!poi)throw Error('地点不可用于当前校区路线');const previous=appliedRouteRef.current?.route_id;onSelectPoi(poi);const mapApplied=new Promise<void>((resolve,reject)=>{setRouteTarget({poiId,revision:Date.now(),onBlocked:message=>reject(new Error(message))});waitObserved(()=>!!appliedRouteRef.current&&appliedRouteRef.current.route_id!==previous&&appliedRouteRef.current.destination_poi_id===poiId,signal,deadline).then(resolve,reject);});const cancel=()=>{setRouteTarget(null);setNavigationEpoch(v=>v+1);};signal.addEventListener('abort',cancel,{once:true});try{await mapApplied;return {routeId:appliedRouteRef.current!.route_id,poiId,mapApplied:true};}catch(error){cancel();throw error;}finally{signal.removeEventListener('abort',cancel);}});return()=>registerHarnessRoute?.(null);});
 const [viewer,setViewer]=useState<TourPhoto|null>(null);
 const [replanArmed,setReplanArmed]=useState(false);
 const [routeTarget,setRouteTarget]=useState<{poiId:string;revision:number;onBlocked?:(message:string)=>void}|null>(null);
