...
def main():
    ...
    if not repo.exists():
        bundled=ROOT/'models/vendor/TripoSR'
        if bundled.is_dir():shutil.copytree(bundled,repo)
        else:run(['git','clone','https://github.com/VAST-AI-Research/TripoSR.git',repo])
    ...
    bundled=ROOT/'models/weights/triposr'
    if (bundled/'model.ckpt').is_file() and (bundled/'config.yaml').is_file():
        (HOME/'weights.json').write_text(json.dumps({'repo':'stabilityai/TripoSR','path':str(bundled),'revision':WEIGHTS}))
    else:
