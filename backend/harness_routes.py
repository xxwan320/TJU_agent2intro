...
async def continuation_ack(rid:str,body:Ack,x_harness_token:str=Header(default='')):
    ...
    ledger=Path(__file__).resolve().parents[1]/'.runtime/harness/run-ledger.jsonl'
    ledger.parent.mkdir(parents=True,exist_ok=True)
    with ledger.open('a',encoding='utf-8') as output:output.write(json.dumps(audit,ensure_ascii=False)+'\n')
    return {'status':'recorded'}
