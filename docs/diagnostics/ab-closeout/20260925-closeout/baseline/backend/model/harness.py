...
class Harness:
    ...
    def begin(self,context):
        ...
        if len(self.runs)>=256:
            ...
            key=(old.context.sessionId,old.context.channel)
            if self.latest.get(key)==(old.context.generation,old.id):self.latest.pop(key,None)
...
async def drive(h,r,message,direct=None):
    try:
        ...
        if direct:
            ...
            if any(w in message for w in ('讲解','暂停','继续','停止')):relevant|={'poi_select','narration_control'}
            if any(w in message for w in ('路线','导航')):relevant.add('route_plan')
            ...
            probe=Path(__file__).resolve().parents[2]/'docs/diagnostics/harness-b/capabilities.json'
            native=json.loads(probe.read_text('utf-8')).get('nativeTools')=='available' if probe.exists() else False
            if native:
        ...
        from pathlib import Path
        path=Path(__file__).resolve().parents[2]/'docs/diagnostics/harness-b/run-ledger.jsonl'
