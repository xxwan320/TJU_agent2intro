...
export class HarnessClient {
 ...
 async cancel(){const a=this.active;this.active=null;if(!a)return;a.abort.abort();if(a.runId)await fetch(`/api/harness/runs/${a.runId}/cancel`,{method:'POST',headers:{'X-Harness-Token':a.token}}).catch(()=>null);}
 async run(context:Omit<ToolContext,'generation'>,message:string,direct:{toolName:string;input:Record<string,unknown>}|undefined,onEvent:(e:HarnessEvent,token:string)=>void,execute:(r:ToolRequest,signal:AbortSignal)=>Promise<ToolResult>){
  ...
  const timer=setTimeout(()=>void this.cancel(),46000);
